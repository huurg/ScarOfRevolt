...
bool Matrix::multCompatible(Matrix& inM) {
...
Matrix Matrix::operator-(Matrix& inM) {
	if((rows==inM.getRows())&&(cols==inM.getCols())) cout << "Warning: subtraction of matrices not same size" << endl;
	Matrix out(rows,cols);
	for(int i = 0; i < rows; i++) {
		for(int j = 0; j < cols; j++) {
			out.E(i,j,E(i,j)-inM.E(i,j));
...
Matrix Matrix::operator*(double inD) {
	Matrix out(rows, cols);
	for(int i = 0; i < rows; i++) {
		for(int j = 0; j < cols; j++) {
			out.E(i,j,E(i,j)*inD);
...
Matrix Matrix::multiply(Matrix& inM) {
	Matrix* out = NULL;
...
Matrix Matrix::tensorProduct(const Matrix& inM) {
...
Matrix Matrix::R(double theta) {
	...
	if(theta==(M_PI/2.0)) {
		...
		double temp = sqrt(2)/2;
		out.E(1,1,-temp,temp);
	} else if(theta==(5.0*M_PI/4.0)) {
		double temp = sqrt(2)/2;
		out.E(1,1,-temp,-temp);
	} else if(theta==(7.0*M_PI/4.0)) {
		double temp = sqrt(2)/2;
		out.E(1,1,temp,-temp);
	} else {
...
Matrix Matrix::Rotate3DX(double inDegrees) {
    Matrix out(3,3);
    out.E(0,0,1);   out.E(0,1,0);   out.E(0,2,0);
    out.E(1,0,0);   out.E(1,1,cos(inDegrees*M_PI/180.0));   out.E(1,2,-sin(inDegrees*M_PI/180.0));
    out.E(2,0,0);   out.E(2,1,sin(inDegrees*M_PI/180.0));   out.E(2,2,cos(inDegrees*M_PI/180.0));
    return out;
}

Matrix Matrix::Rotate3DY(double inDegrees) {
    Matrix out(3,3);
    out.E(0,0,sin(inDegrees*M_PI/180.0));   out.E(0,1,0);   out.E(0,2,cos(inDegrees*M_PI/180.0));
    out.E(1,0,0);   out.E(1,1,1);   out.E(1,2,0);
    out.E(2,0,cos(inDegrees*M_PI/180.0));   out.E(2,1,0);   out.E(2,2,-sin(inDegrees*M_PI/180.0));
    return out;
}

Matrix Matrix::Rotate3DZ(double inDegrees) {
    Matrix out(3,3);
    out.E(0,0,cos(inDegrees*M_PI/180.0));   out.E(0,1,-sin(inDegrees*M_PI/180.0));  out.E(0,2,0);
    out.E(1,0,sin(inDegrees*M_PI/180.0));   out.E(1,1,cos(inDegrees*M_PI/180.0));   out.E(1,2,0);
    out.E(2,0,0);   out.E(2,1,0);   out.E(2,2,1);
    return out;
}
